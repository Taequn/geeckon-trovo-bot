AUTH_URL = 'https://open.trovo.live/page/login.html'

API_URL = 'https://open-api.trovo.live/openplatform'

VALIDATE_URL = f'{API_URL}/validate'
REVOKE_URL = f'{API_URL}/revoke'
REFRESH_URL = f'{API_URL}/refreshtoken'

GAME_CATEGORIES_URL = f'{API_URL}/categorys/top'
SEARCH_CATEGORIES_URL = f'{API_URL}/searchcategory'
TOP_CHANNELS_URL = f'{API_URL}/gettopchannels'
GET_USERS_URL = f'{API_URL}/getusers'
GET_USER_INFO_URL = f'{API_URL}/getuserinfo'

GET_CHANNEL_INFO_URL = f'{API_URL}/channels/id'
READ_CHANNEL_INFO_URL = f'{API_URL}/channel'
EDIT_CHANNEL_INTO_UR = f'{API_URL}/channels/update'

GET_EMOTES_URL = f'{API_URL}/getemotes'
GET_LIVESTREAMS_URL = f'{API_URL}/livestreamurl'
GET_CLIPS_INFO_URL = f'{API_URL}/clips'
GET_PAST_STREAMS_URL = f'{API_URL}/paststreams'

GET_DROPS_URL = f'{API_URL}/getdropsentitlements'
UPDATE_DROPS_URL = f'{API_URL}/updatedropsentitlements'

CHAT_SEND_URL = f'{API_URL}/chat/send'
CHAT_COMMAND_URL = f'{API_URL}/channels/command'

EXCHANGE_TOKEN_URL = f'{API_URL}/exchangetoken'

